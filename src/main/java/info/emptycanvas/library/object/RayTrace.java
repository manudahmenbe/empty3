...
/* [ la couleur finale du pixel courant, en lui passant le rayon primaire �mis.                           ] */
    CColor RayTrace(const CScene&scene, const CRay&ray, unsigned int depth) {
        ...
        // Eclairage
        bool lightBlocked;        // Booleen qui nous permet de dire si le rayon de lumi�re est bloqu� sur son chemin ou non
        ...
        if (closestNode) {
            ...
            for (int i = 0; i < scene.GetNumLights(); i++) {
                ...
                for (int j = 0; j < scene.GetNumNodes(); j++) {
                    ...
                    {
                        lightToInterDist = (lightInterInfo.mIntersection - scene.GetLight(i)->GetPosition()).
                        GetMagnitude();
                ...
                if (!lightBlocked)
                    finalColor += scene.GetLight(i)->
                GetLightAt(closestInterInfo.mNormal, closestInterInfo.mIntersection, closestInterInfo.mMaterial);
            }

            // Clean non permanent material
            if (closestInterInfo.mMaterial->GetPermanency() == false)
            delete(closestInterInfo.mMaterial);
            closestInterInfo.mMaterial = NULL;
        }
    ...
    int Render(const CScene&scene, unsigned int width, unsigned height, const char*outputfilename) {
        ...
        for (int y = 0; y < height; y++)
            for (int x = 0; x < width; x++) {
                ...
                fwrite( & tmpR, sizeof(unsignedchar),1, mOutputFileRAW)
                fwrite( & tmpG, sizeof(unsignedchar),1, mOutputFileRAW)
                fwrite( & tmpB, sizeof(unsignedchar),1, mOutputFileRAW)
            }
