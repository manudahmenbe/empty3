...
public abstract class ITexture extends MediaListenerAdapter {
    ...
    public be.manudahmen.empty3.library.tribase.Point getCoord(double x, double y) {
        be.manudahmen.empty3.library.tribase.Point p = new be.manudahmen.empty3.library.tribase.Point(x, y);
        if (getColorMask() == COLOR_IDENT)
            return p;
        if ((getColorMask() & COLOR_MIROR_X) > 0) {
            p = new be.manudahmen.empty3.library.tribase.Point(1.0 - p.x, p.y);
        }
        if ((getColorMask() & COLOR_MIROR_Y) > 0) {
            p = new be.manudahmen.empty3.library.tribase.Point(p.x, 1.0 - p.y);
        }
        if ((getColorMask() & COLOR_MIROR_XY) > 0) {
            p = new be.manudahmen.empty3.library.tribase.Point(p.y, p.x);
        }
        if ((getColorMask() & COLOR_ROT_090) > 0) {
            p = new be.manudahmen.empty3.library.tribase.Point(1.0 - p.y, p.x);
        }
        if ((getColorMask() & COLOR_ROT_180) > 0) {
            p = new be.manudahmen.empty3.library.tribase.Point(1.0 - p.x, 1.0 - p.x);
        }
        if ((getColorMask() & COLOR_ROT_270) > 0) {
            p = new be.manudahmen.empty3.library.tribase.Point(p.y, 1.0 - p.x);
        }
        return p;
    }
