...
public class Polygons extends SurfaceParametriquePolynomiale {
    ...
    public Point3D calculerPoint3D(double u, double v) {
        int u0 = (int)(u*coefficients.data2d.size());
        int v0 = (int)(v*coefficients.getData2d().get(0).size());
        int u1 = u0+1;
        int v1 = v0+1;
        ...
        double U = Math.IEEEremainder(u, coefficients.getData2d().size());
        double V = Math.IEEEremainder(v, coefficients.getData2d().get(0).size());
        ...
        return points[0].plus(pUv0.plus(pUv1).mult(V)); // Discuttable
    }
}
