...
import tests2.balleclou.BalleClous2;
...
public class TestBalleCouleur001 extends TestObjetSub {
    ...
    public void testScene() throws Exception {
        ...
        for (int j = 0; j < s.length; j++) {
            ...
            tubulaireN2.setDiameter((float) TUBE_RAYON);
            tubulaireN2.texture(new TextureImg(new ECBufferedImage(ImageIO.read(new File("samples/img/PHOTO-NU.jpg")))));


            tubulaireN2.setSoulCurve(gdx_BSplineCurve);

            scene().add(tubulaireN2);

//ballec.be.manudahmen.empty3.tests2.position().be.manudahmen.empty3.tests2.rotation = ballec.be.manudahmen.empty3.tests2.position().be.manudahmen.empty3.tests2.rotation.mult(matrix1(totalA, totalB));
